...
export class SocialSimulation {
  ...
  updatePassengers(passengers: PassengerState[], deltaTime: number): number {
    let totalStress = 0;
  ...
  private updatePassengerState(
    ...
    for (const nearby of nearbyPassengers) {
      ...
      if (trigger) {
        passenger.stress = Math.max(0, Math.min(100,
          passenger.stress + trigger.stressModifier * deltaTime
        ));
      }

      // Apply social energy from nearby passengers
      const nearbyTemplate = PASSENGER_TEMPLATES[nearby.type];
      passenger.mood = Math.max(0, Math.min(100,
        passenger.mood + nearbyTemplate.socialEnergy * deltaTime * 0.5
      ));
    }
    ...
    if (passenger.type === PassengerType.COUPLE_LEFT ||
      ...
      if (partner) {
        const distance = Math.hypot(
          passenger.position.x - partner.position.x,
          passenger.position.y - partner.position.y
        );
    ...
    const crowdCount = nearbyPassengers.filter(p =>
      this.getDistance(passenger, p) < template.bubbleSize
    ).length;
  ...
  private getDistance(p1: PassengerState, p2: PassengerState): number {
    return Math.hypot(
  ...
  checkGameOver(passengers: PassengerState[]): boolean {
