import { GameState, PassengerState, Seat, Position } from './types';
...
export class Renderer {
  ...
  render(state: GameState, coach: TrainCoach): void {
    this.clear();
  ...
  private drawCoach(coach: TrainCoach): void {
    // Draw floor
  ...
  private drawSeats(seats: Seat[]): void {
    for (const seat of seats) {
      this.ctx.fillStyle = seat.occupied ? '#666' : '#8a8a8a';
      this.ctx.strokeStyle = '#aaa';
      this.ctx.lineWidth = 2;

      const seatSize = 35;
      this.ctx.fillRect(
        seat.position.x - seatSize / 2,
        seat.position.y - seatSize / 2,
        seatSize,
        seatSize
      );
      this.ctx.strokeRect(
  ...
  private drawQueue(queue: PassengerState[], selected: PassengerState | null): void {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      // Draw border
      this.ctx.strokeStyle = isSelected ? '#ff0' : '#aaa';
      this.ctx.lineWidth = isSelected ? 3 : 1;
      this.ctx.strokeRect(queueX, y, 25, 25);
    }
  }

  private drawPlacementPreview(passenger: PassengerState): void {
    // This would show where the passenger will go when you click
    // For now, just highlight the selected passenger in queue
  }

  drawPassengerHoverInfo(passenger: PassengerState, mousePos: Position): boolean {
    const distance = Math.hypot(
      passenger.position.x - mousePos.x,
      passenger.position.y - mousePos.y
    );
